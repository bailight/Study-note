...
SuperAgent是轻量流行的 AJAX 请求库（fetch或$.ajax的替代方案），具有链式（fluent）API，可读性强。
...
*   **Определение:** Это **препроцессоры** CSS. Они расширяют стандартный CSS переменными, вложенностью, миксинами (функциями) и логикой, а затем **транслируются (компилируются)** в обычный CSS, который понимают браузеры.
*   **Ключевые особенности:**
    *   **Переменные:** `$primary-color: #333;` (Sass/SCSS) или `@primary-color: #333;` (LESS).
    *   **Вложенность:** Селекторы можно вкладывать друг в друга, что отражает структуру HTML.
    *   **Миксины (Mixins):** Переиспользуемые блоки стилей, похожие на функции.
    *   **Импорт:** Можно разбивать стили на файлы и импортировать их.
    *   **Математические операции.**
*   **Сравнение:**
    *   **Sass** и **SCSS** — это по сути один препроцессор с двумя синтаксисами. У Sass свой, более лаконичный синтаксис (без фигурных скобок и точек с запятой). **SCSS** — это надмножество CSS, то есть любой валидный CSS файл является валидным SCSS-файлом. SCSS стал стандартом де-факто.
    *   **LESS** похож на SCSS по возможностям, но использует символ `@` для переменных.
*   **Совместимость:** Полная. Поскольку итоговый результат — обычный CSS, совместимость 100% с любым браузером, который понимает CSS.

---

### 7. Клиентские сценарии. JavaScript

*   **Особенности:** Код выполняется **на стороне клиента** (в браузере пользователя). Не имеет доступа к файловой системе сервера или БД (за исключением API через HTTP-запросы).
*   **Сферы применения:**
    *   Манипуляция DOM (анимация, динамическое изменение контента).
    *   Обработка событий (клики, наведение, ввод с клавиатуры).
    *   Валидация форм перед отправкой.
    *   Отправка асинхронных запросов (AJAX).
    *   Работа с Cookies и Local Storage.
*   **Язык JavaScript:** Высокоуровневый, интерпретируемый, мультипарадигмальный (ООП, функциональное программирование) язык с динамической типизацией.

---

### 8. Версии ECMAScript (ES6 и ES7)

ECMAScript — это стандарт, спецификация. JavaScript — его самая популярная реализация.

*   **ES6 (ES2015) — огромное обновление:**
    *   `let` и `const` для объявления переменных с блочной областью видимости.
    *   Стрелочные функции: `() => {}` (короткий синтаксис и сохранение контекста `this`).
    *   Классы: `class MyClass { constructor() {} }` (синтаксический сахар над прототипами).
    *   Promises (Обещания) для удобной работы с асинхронностью.
    *   Шаблонные строки: `` `Hello, ${name}!` ``.
    *   Деструктуризация: `const { prop } = object;`.
    *   Параметры по умолчанию.
    *   Модули (`import`/`export`).

*   **ES7 (ES2016) — меньше нововведений:**
    *   Метод `Array.prototype.includes()`.
    *   Оператор возведения в степень `**`.

---

### 9. Синхронная и асинхронная обработка. AJAX

*   **Синхронная:** Браузер отправляет запрос и **блокирует** интерфейс пользователя, ожидая ответа от сервера. Плохой пользовательский опыт.
*   **Асинхронная (AJAX):** Браузер отправляет запрос в фоновом режиме **не блокируя** интерфейс. Когда ответ приходит, обрабатывает его с помощью callback-функций. Пользователь продолжает работать со страницей.
*   **AJAX (Asynchronous JavaScript and XML):** Подход к созданию асинхронных веб-приложений. Не зависит от формата данных (сейчас чаще используется JSON, а не XML). Основа современного веба (SPA).

---

### 10. Библиотека jQuery

*   **Назначение:** В прошлом — главная библиотека для упрощения JS-разработки. Сглаживала различия между браузерами и предоставляла простой API для частых задач.
*   **Основные API:**
    *   **Работа с DOM:** `$('селектор')` — выбор элементов, `.html()`, `.css()`, `.addClass()`, `.on('click', ...)`.
    *   **AJAX:** `$.ajax()`, `$.get()`, `$.post()`.
*   **Использование для AJAX и DOM:** jQuery делает эти задачи чрезвычайно простыми по сравнению с нативным JS (особенно старых версий).
*   **Сейчас:** Необходимость в jQuery снизилась due to modern browsers и стандартам (ES6+, Fetch API, `document.querySelector`).

---

### 11. Реализация AJAX с помощью SuperAgent

**SuperAgent** — легкая и популярная библиотека для AJAX-запросов (альтернатива `fetch` или `$.ajax`). Имеет цепочечный (fluent) API, очень читаемый.
...
superagent
  .get('/api/user')
  .query({ id: 123 }) // добавить параметры в URL
  .then(response => {
...
superagent
  .post('/api/user')
  .send({ name: 'John', email: 'john@example.com' }) // отправить данные JSON
  .set('X-API-Key', 'foobar') // установить заголовок
  .then(response => { ... });
```

---

### 12. Серверные сценарии. CGI

*   **Определение:** CGI (Common Gateway Interface) — **стандарт**, который определяет, как веб-сервер (Apache, Nginx) взаимодействует с внешними программами.
*   **Назначение:** Позволить веб-серверу генерировать динамический контент, запуская программы на Perl, Python, C++ и т.д.
*   **Ключевые особенности:**
    *   Для **каждого запроса** веб-сервер порождает **новый процесс** CGI-программы.
    *   Передает данные запроса через переменные окружения и stdin.
    *   Получает ответ от stdout программы и отправляет клиенту.
    *   **Недостаток:** Очень высокие накладные расходы из-за создания процесса на каждый запрос. Медленно.

---

### 13. FastCGI

*   **Особенности:** Эволюция CGI, призванная решить проблему производительности.
    *   CGI-программа запускается **один раз** и работает как **долгоживущий процесс**.
    *   Веб-сервер общается с этим процессом через socket или Unix-сокеты.
    *   Один процесс обрабатывает множество запросов.
*   **Преимущества перед CGI:**
    *   **Высокая производительность:** Нет накладных расходов на запуск процесса для каждого запроса.
    *   **Масштабируемость.**
*   **Недостатки:** Более сложная архитектура и настройка по сравнению с чистым CGI.
